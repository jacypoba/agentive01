...
const LANGUAGE_SIGNALS: Record<SupportedLanguage, SignalDefinition[]> = {
  ...
  pt: [
    phrase("estou_procurando_uma", /\bestou procurando uma\b/i),
    phrase("estou_procurando_um", /\bestou procurando um\b/i),
    phrase("estou_procurando", /\bestou procurando\b/i),
    ...
    phrase("procuro_um", /\bprocuro um\b/i),
    phrase("ola", /(?:^|[^\p{L}])ol[aá](?:[^\p{L}]|$)/iu),
    stopword("pt_fn", /\b(de|um|uma|em|para|que|com|estou|preciso|quero|até|por)\b/i),
    stopword("pt_verb", /\b(procuro|procurar|procurando|comprar|arrendar|moradia)\b/i),
